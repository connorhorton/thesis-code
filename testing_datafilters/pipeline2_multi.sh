...
echo "pairsamtools!" 
pairsamtools parse -c helper_files/$GENOME.chrom.sizes.mainonly --min-mapq 0 --assembly $GENOME --output-stats $DIR/${FASTQ_ROOT}_stats.txt $DIR/${FASTQ_ROOT}_raw.sam | {
...
#remove promiscuous
# if [ $DIPLOID ] ; then
# 	python remove_promiscuous.py $FASTQ_ROOT
# fi
echo "zipping and indexing pairs file"
bgzip -f $DIR/$FASTQ_ROOT.nodups.pairs
pairix -f $DIR/$FASTQ_ROOT.nodups.pairs.gz
#run pairsqc!
REPORT_DIR=$(echo $DIR | rev | cut -f 1 -d '/' | rev) # get report directory for PairsQCp
