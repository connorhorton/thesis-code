...
# fi
echo "filtering out undigested fragments"
python filter_undigested.py $DIR/${FASTQ_ROOT}.nodups.pairs $DIR/${FASTQ_ROOT}.accepted.pairs $DIR/${FASTQ_ROOT}.discarded.pairs
echo "zipping and indexing pairs file"
bgzip -f $DIR/$FASTQ_ROOT.accepted.pairs
pairix -f $DIR/$FASTQ_ROOT.accepted.pairs.gz
#run pairsqc!
REPORT_DIR=$(echo $DIR | rev | cut -f 1 -d '/' | rev) 
echo "running PairsQC"
if [[ $GENOME == "mm10" ]]; then
		python $PAIRSQC_PATH/pairsqc.py -p $DIR/$FASTQ_ROOT.accepted.pairs.gz -c helper_files/$GENOME.chrom.sizes.mainonly -t P -s $REPORT_DIR/$FASTQ_ROOT -M 8.2
elif [[ $GENOME == "hg38" ]]; then
		python $PAIRSQC_PATH/pairsqc.py -p $DIR/$FASTQ_ROOT.accepted.pairs.gz -c helper_files/$GENOME.chrom.sizes.mainonly -t P -s $REPORT_DIR/$FASTQ_ROOT
fi
echo "calculating gini index"
cooler cload pairix helper_files/$GENOME.chrom.sizes.mainonly:1000000 $DIR/$FASTQ_ROOT.accepted.pairs.gz $DIR/$FASTQ_ROOT.gini.cool 2>> $DIR/${FASTQ_ROOT}_std.err
python gini.py $DIR/$FASTQ_ROOT.gini.cool report/$REPORT_DIR/$FASTQ_ROOT.cis_to_trans.out
echo "generating cool files"
cooler cload pairix helper_files/$GENOME.chrom.sizes.mainonly:10000 $DIR/$FASTQ_ROOT.accepted.pairs.gz $DIR/$FASTQ_ROOT.cool 2>> $DIR/${FASTQ_ROOT}_std.err
...
rm $DIR/${FASTQ_ROOT}.discarded.pairs